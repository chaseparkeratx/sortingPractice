...
const stringsToNumbs = (numbers) => {
  const numberArray = strNums.map(string => parseInt(string))
...
const sumEvens = () => {
  const sumOfEvens = strNums.reduce((accumulator, currentValue) => {
    if(currentValue % 2 === 0) {
...
const weather = [
  ...
  const idealTemp = 15.915;
  const minTemp = weather.filter(temp => temp.min_temp === idealTemp)
  console.log(minTemp)